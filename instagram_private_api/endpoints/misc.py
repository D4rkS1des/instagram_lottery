...
class MiscEndpointsMixin(object):
    ...
    def direct_v2_threads(self, thread_id=None, cursor=None):
        """
        Get v2 inbox messages in thread
        """
        query = {}
        if thread_id:
            query = {
            ...
            if cursor:
                query.update({'cursor': cursor})

        return self._call_api('direct_v2/threads/' + thread_id + '/', query=query)
